...
use crate::media_ingest::{
    AudioIngestPolicy, CanonicalAudioProfile, MediaIngestError, MediaIngestRequest,
};
use crate::persistence::MediaStorageError;
...
pub async fn create_media(
    ...
    let ingested = state
        ...
        .ingest(
            ...
            AudioIngestPolicy::media_upload(resolve_audio_upload_limit_bytes()),
            CanonicalAudioProfile::SpeechRecognition16KhzMonoWav,
        )
        .await
        .map_err(map_media_ingest_error)?;
    let path = ingested.original_storage_key;
    let media_asset_id = ingested.source_asset.map(|asset| asset.id);
    let (canonical_media_asset_id, canonical_path, canonical_url) = ingested
        .canonical
        .map(|canonical| {
            let url = media_url(&canonical.storage_key);
            (
                Some(canonical.asset.id),
                Some(canonical.storage_key),
                Some(url),
            )
        })
    ...
    Ok(Json(UploadMediaResponse {
        ...
        content_type,
        filename,
        size_bytes: ingested.source_size_bytes,
        media_asset_id,
...
fn map_media_ingest_error(err: MediaIngestError) -> ApiError {
    if err.is_invalid_input() {
        ...
        ApiError::internal(format!("Media ingest failed: {err}"))
    }
}
