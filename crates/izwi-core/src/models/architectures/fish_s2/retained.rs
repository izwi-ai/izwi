...
use super::{
    ...
    sanitize_temperature, sanitize_top_p, FishS2ConditioningPrompt, FishS2GenerationOutput,
    FishS2GenerationParams, FishS2Reference, FishS2Sampler, FishS2SemanticSampler,
    FishS2SlowOutput, FishS2TtsGenerationDiagnostics, FishS2TtsModel, FishS2VqCodes,
    FISH_S2_FAST_SAMPLER_SEED, FISH_S2_SEMANTIC_SAMPLER_SEED, RAS_WIN_SIZE,
};
...
struct FishS2RetainedCheckpointPayload {
    ...
    fast_cache: Option<PhysicalPagedKvCache>,
    slow_position: usize,
    fast_position: usize,
    slow_output: Option<FishS2SlowOutput>,
    semantic_sampler: FishS2SemanticSampler,
...
impl FishS2TtsModel {
    ...
    pub(crate) fn new_retained_state(
        ...
        if slow_cache.context_len() != 0 || fast_cache.context_len() != 0 {
            return Err(Error::InvalidInput(
        ...
        let max_frames = params
            .max_frames
            .min(self.config.max_seq_len - artifact.prompt.prompt_length);
        if max_frames == 0 {
            return Err(Error::InvalidInput(
                "Fish S2 prompt leaves no room for generated audio frames".into(),
            ));
        }
        ...
        Ok(FishS2RetainedState {
            ...
            fast_position: 0,
            slow_output: None,
            semantic_sampler: FishS2SemanticSampler::new(
                temperature,
                top_p,
                FISH_S2_SEMANTIC_SAMPLER_SEED,
            ),
            fast_sampler: FishS2Sampler::new(temperature, top_p, FISH_S2_FAST_SAMPLER_SEED),
    ...
    pub(crate) fn new_retained_state_in_quantum(
        ...
        let mut state = self.new_retained_state(artifact, params, slow_cache, fast_cache)?;
    ...
    pub(crate) fn retained_decode_step(
        ...
        if state.finished {
            return Ok(FishS2RetainedStep::Finished {
                frames_generated: state.frames_generated(),
            });
        }
        let runtime = self.native_runtime()?;
        if state.pending_semantic.is_none() {
            ...
            state.fast_cache.reset_invocation()?;
            let semantic_code = super::semantic_code_from_token_id(&self.config, semantic)?;
            let hidden = runtime.fast.project_slow_hidden(&slow.hidden_states)?;
            let _ = runtime
                .fast
                .forward_step(&hidden, 0, &mut state.fast_cache)?;
            state.pending_semantic = Some(semantic);
            state.pending_codes = vec![semantic_code];
            state.fast_position = 1;
            state.completions_drained = false;
            return state.stage(FishS2RetainedStep::Codebook {
                frame: state.frames_generated(),
                codebook_position: 0,
            });
        }
        if state.fast_position < self.config.num_codebooks {
            let current = runtime
                .fast
                .codebook_embedding(*state.pending_codes.last().unwrap())?;
            let logits =
                runtime
                    .fast
                    .forward_step(&current, state.fast_position, &mut state.fast_cache)?;
            let code = super::fast::sample_logits(&logits.i((0, 0))?, &mut state.fast_sampler)?;
            let position = state.fast_position;
            state.pending_codes.push(code);
            state.fast_position += 1;
            state.completions_drained = false;
            return state.stage(FishS2RetainedStep::Codebook {
                frame: state.frames_generated(),
                codebook_position: position,
            });
        }
        let semantic = state.pending_semantic.take().unwrap();
        let frame = super::FishS2GeneratedFrame {
            semantic_token_id: semantic,
            codebooks: std::mem::take(&mut state.pending_codes),
        };
    ...
    pub(crate) fn finalize_retained_state(
        ...
        self.validate_retained_state(state)?;
        if state.active_quantum.is_some() || state.pending_semantic.is_some() {
            return Err(Error::InferenceError(
                "Fish S2 finalize requires a committed frame boundary".into(),
            ));
        }
        ...
        let started = Instant::now();
        let samples = self.native_runtime()?.dac.decode_vq_codes(&FishS2VqCodes {
...
impl FishS2RetainedState {
    pub(crate) fn begin_managed_quantum(
        ...
        if slow_cache.arena().id() != self.slow_cache.arena().id()
            || fast_cache.arena().id() != self.fast_cache.arena().id()
            || slow_cache.context_len() != self.slow_position
            || fast_cache.context_len() != self.fast_cache.context_len()
        {
            return Err(Error::InvalidInput(
        ...
        Ok(FishS2RetainedCheckpoint {
            ...
            payload: Some(FishS2RetainedCheckpointPayload {
                ...
                fast_cache: Some(std::mem::replace(&mut self.fast_cache, fast_cache)),
                slow_position: self.slow_position,
                fast_position: self.fast_position,
                ...
                pending_codes: self.pending_codes.clone(),
    ...
    pub(crate) fn rollback_managed_quantum(
        ...
        self.fast_cache = payload.fast_cache.ok_or_else(|| {
            Error::InferenceError("initial Fish S2 state must be discarded on rollback".into())
        })?;
        self.slow_position = payload.slow_position;
        self.fast_position = payload.fast_position;
        self.slow_output = payload.slow_output;
        self.semantic_sampler = payload.semantic_sampler;
        self.fast_sampler = payload.fast_sampler;
        self.pending_semantic = payload.pending_semantic;
        self.pending_codes = payload.pending_codes;
    ...
    pub(crate) fn take_managed_write_completions(&mut self) -> Vec<Arc<KvWriteBatchCompletion>> {
        let mut completions = self.slow_cache.take_completed_writes();
        completions.extend(self.fast_cache.take_completed_writes());
    ...
    pub(crate) const fn fast_position(&self) -> usize {
        self.fast_position
    }
...
mod tests {
    ...
    fn state() -> FishS2RetainedState {
        FishS2RetainedState {
            ...
            fast_cache: test_physical_cache(92, 1, 1, 1, 4),
            slow_position: 0,
            fast_position: 0,
            slow_output: None,
    ...
    #[test]
    fn managed_rollback_restores_independent_slow_and_fast_clocks() {
        let mut state = state();
        let slow = test_physical_cache(91, 1, 1, 1, 8);
        let fast = test_physical_cache(92, 1, 1, 1, 4);
        let mut checkpoint = state.begin_managed_quantum(slow, fast).unwrap();
        state.slow_position = 3;
        state.fast_position = 2;
        state.pending_semantic = Some(42);
        state.pending_codes = vec![1, 2];
        state.staged_step = Some(FishS2RetainedStep::Codebook {
            frame: 0,
            codebook_position: 1,
        });
        state.rollback_managed_quantum(&mut checkpoint).unwrap();
        assert_eq!(state.slow_position(), 0);
        assert_eq!(state.fast_position(), 0);
        assert!(state.pending_semantic.is_none());
