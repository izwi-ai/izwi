...
#[cfg(feature = "metal")]
const PARAKEET_METAL_SOURCE: &str = r#"
#include <metal_stdlib>
using namespace metal;

kernel void parakeet_depthwise_conv1d_f32(
...
kernel void parakeet_depthwise_conv1d_bias_swish_f32(
    device const float* input [[buffer(0)]],
    device const float* weight [[buffer(1)]],
    ...
    constant uint& dilation [[buffer(11)]],
    uint gid [[thread_position_in_grid]]
) {
    uint total = batch * channels * output_width;
    if (gid >= total) {
    ...
    float z = acc + bias[channel];
    output[gid] = z / (1.0f + exp(-z));
}

kernel void parakeet_depthwise_conv2d_f32(
    device const float* input [[buffer(0)]],
    device const float* weight [[buffer(1)]],
    device float* output [[buffer(2)]],
    constant uint& batch [[buffer(3)]],
    constant uint& channels [[buffer(4)]],
    constant uint& input_height [[buffer(5)]],
    constant uint& input_width [[buffer(6)]],
    ...
    constant uint& dilation [[buffer(13)]],
    uint gid [[thread_position_in_grid]]
) {
...
#[cfg(feature = "metal")]
#[derive(Debug, Clone, Copy)]
struct DepthwiseConv1dBiasSwishOp {
    padding: usize,
    stride: usize,
    dilation: usize,
}
...
#[cfg(feature = "metal")]
impl CustomOp3 for DepthwiseConv1dBiasSwishOp {
    fn name(&self) -> &'static str {
        "parakeet-depthwise-conv1d-bias-swish-metal"
    }
    ...
    fn cpu_fwd(
        ...
        bail!("parakeet-depthwise-conv1d-bias-swish-metal requires a Metal tensor")
    }
    ...
    fn metal_fwd(
        ...
        {
            bail!("parakeet-depthwise-conv1d-bias-swish-metal only supports F32 tensors")
        }
        if !input_layout.is_contiguous()
            || !weight_layout.is_contiguous()
            || !bias_layout.is_contiguous()
        {
            bail!("parakeet-depthwise-conv1d-bias-swish-metal requires contiguous inputs")
        }

        let input_dims = input_layout.dims();
        let weight_dims = weight_layout.dims();
        let bias_dims = bias_layout.dims();
        if input_dims.len() != 3 || weight_dims.len() != 3 || bias_dims.len() != 1 {
            bail!("parakeet-depthwise-conv1d-bias-swish-metal expects ranks [3,3,1]")
        }
        ...
        if out_channels != channels || in_per_group != 1 || bias_dims[0] != channels {
            bail!("parakeet-depthwise-conv1d-bias-swish-metal expects [B,C,T], [C,1,K], [C]")
        }
        if self.stride == 0 || self.dilation == 0 {
            bail!("parakeet-depthwise-conv1d-bias-swish-metal received an invalid stride/dilation")
        }
        if input_width + 2 * self.padding < self.dilation * (kernel_width - 1) + 1 {
            bail!("parakeet-depthwise-conv1d-bias-swish-metal output width would underflow")
        }
        ...
        let output = device.new_buffer(
            elem_count,
            DType::F32,
            "parakeet-depthwise-conv1d-bias-swish",
        )?;
        let encoder = device.command_encoder()?;
        encoder.set_label("parakeet-depthwise-conv1d-bias-swish");
        let pipeline = depthwise_conv1d_bias_swish_pipeline(device.metal_device())?;
        ...
        encoder.set_bytes(6, &(input_width as u32));
        encoder.set_bytes(7, &(output_width as u32));
        encoder.set_bytes(8, &(kernel_width as u32));
        encoder.set_bytes(9, &(self.padding as u32));
...
impl CustomOp2 for DepthwiseConv1dOp {
    fn name(&self) -> &'static str {
        "parakeet-depthwise-conv1d-metal"
    }
    ...
    fn cpu_fwd(
        ...
        bail!("parakeet-depthwise-conv1d-metal requires a Metal tensor")
    }
    ...
    fn metal_fwd(
        ...
        if input_storage.dtype() != DType::F32 || weight_storage.dtype() != DType::F32 {
            bail!("parakeet-depthwise-conv1d-metal only supports F32 tensors")
        }
        if !input_layout.is_contiguous() || !weight_layout.is_contiguous() {
            bail!("parakeet-depthwise-conv1d-metal requires contiguous inputs")
        }

        let input_dims = input_layout.dims();
        let weight_dims = weight_layout.dims();
        if input_dims.len() != 3 || weight_dims.len() != 3 {
            bail!("parakeet-depthwise-conv1d-metal expects rank-3 tensors")
        }
        ...
        if out_channels != channels || in_per_group != 1 {
            bail!("parakeet-depthwise-conv1d-metal expects depthwise weights shaped [C,1,K]")
        }
        if self.stride == 0 || self.dilation == 0 {
            bail!("parakeet-depthwise-conv1d-metal received an invalid stride/dilation")
        }
        if input_width + 2 * self.padding < self.dilation * (kernel_width - 1) + 1 {
            bail!("parakeet-depthwise-conv1d-metal output width would underflow")
        }
        ...
        let device = input_storage.device().clone();
        let output = device.new_buffer(elem_count, DType::F32, "parakeet-depthwise-conv1d")?;
        let encoder = device.command_encoder()?;
        encoder.set_label("parakeet-depthwise-conv1d");
...
#[cfg(feature = "metal")]
impl CustomOp2 for DepthwiseConv2dOp {
    fn name(&self) -> &'static str {
        "parakeet-depthwise-conv2d-metal"
    }
    ...
    fn cpu_fwd(
        ...
        bail!("parakeet-depthwise-conv2d-metal requires a Metal tensor")
    }
    ...
    fn metal_fwd(
        ...
        if input_storage.dtype() != DType::F32 || weight_storage.dtype() != DType::F32 {
            bail!("parakeet-depthwise-conv2d-metal only supports F32 tensors")
        }
        if !input_layout.is_contiguous() || !weight_layout.is_contiguous() {
            bail!("parakeet-depthwise-conv2d-metal requires contiguous inputs")
        }

        let input_dims = input_layout.dims();
        let weight_dims = weight_layout.dims();
        if input_dims.len() != 4 || weight_dims.len() != 4 {
            bail!("parakeet-depthwise-conv2d-metal expects rank-4 tensors")
        }
        ...
        if out_channels != channels || in_per_group != 1 {
            bail!("parakeet-depthwise-conv2d-metal expects depthwise weights shaped [C,1,Kh,Kw]")
        }
        if self.stride == 0 || self.dilation == 0 {
            bail!("parakeet-depthwise-conv2d-metal received an invalid stride/dilation")
        }
        if input_height + 2 * self.padding < self.dilation * (kernel_height - 1) + 1
            || input_width + 2 * self.padding < self.dilation * (kernel_width - 1) + 1
        {
            bail!("parakeet-depthwise-conv2d-metal output shape would underflow")
        }
        ...
        let device = input_storage.device().clone();
        let output = device.new_buffer(elem_count, DType::F32, "parakeet-depthwise-conv2d")?;
        let encoder = device.command_encoder()?;
        encoder.set_label("parakeet-depthwise-conv2d");
        let pipeline = depthwise_conv2d_pipeline(device.metal_device())?;
        encoder.set_compute_pipeline_state(&pipeline);
        ...
        encoder.set_bytes(3, &(batch as u32));
        encoder.set_bytes(4, &(channels as u32));
        encoder.set_bytes(5, &(input_height as u32));
        encoder.set_bytes(6, &(input_width as u32));
        encoder.set_bytes(7, &(output_height as u32));
        encoder.set_bytes(8, &(output_width as u32));
        encoder.set_bytes(9, &(kernel_height as u32));
        encoder.set_bytes(10, &(kernel_width as u32));
...
fn depthwise_conv1d_pipeline(device: &MetalDevice) -> CandleResult<ComputePipeline> {
    ...
    let library = device
        .new_library_with_source(PARAKEET_METAL_SOURCE, None)
        .map_err(candle_core::Error::wrap)?;
    let function = library
        .get_function("parakeet_depthwise_conv1d_f32", None)
        .map_err(candle_core::Error::wrap)?;
    let pipeline = device
...
#[cfg(feature = "metal")]
fn depthwise_conv1d_bias_swish_pipeline(device: &MetalDevice) -> CandleResult<ComputePipeline> {
    static PIPELINES: OnceLock<Mutex<HashMap<u64, ComputePipeline>>> = OnceLock::new();
    let registry_id = device.registry_id();
    let pipelines = PIPELINES.get_or_init(|| Mutex::new(HashMap::new()));
    ...
    let library = device
        ...
        .map_err(candle_core::Error::wrap)?;
    let function = library
        .get_function("parakeet_depthwise_conv1d_bias_swish_f32", None)
        .map_err(candle_core::Error::wrap)?;
    let pipeline = device
        .new_compute_pipeline_state_with_function(&function)
        .map_err(candle_core::Error::wrap)?;
...
#[cfg(feature = "metal")]
fn depthwise_conv2d_pipeline(device: &MetalDevice) -> CandleResult<ComputePipeline> {
    static PIPELINES: OnceLock<Mutex<HashMap<u64, ComputePipeline>>> = OnceLock::new();
    let registry_id = device.registry_id();
    let pipelines = PIPELINES.get_or_init(|| Mutex::new(HashMap::new()));
    ...
    let library = device
        ...
        .map_err(candle_core::Error::wrap)?;
    let function = library
        .get_function("parakeet_depthwise_conv2d_f32", None)
        .map_err(candle_core::Error::wrap)?;
    let pipeline = device
        .new_compute_pipeline_state_with_function(&function)
        .map_err(candle_core::Error::wrap)?;
...
pub fn depthwise_conv1d_enabled_for_device(device: &candle_core::Device) -> bool {
    device.is_metal()
        && std::env::var("IZWI_PARAKEET_METAL_DEPTHWISE_CONV1D")
            .ok()
            .map(|value| {
                matches!(
...
pub fn depthwise_conv2d_enabled_for_device(device: &candle_core::Device) -> bool {
    device.is_metal()
        && std::env::var("IZWI_PARAKEET_METAL_DEPTHWISE_CONV2D")
            .ok()
            .map(|value| {
                matches!(
                    value.trim().to_ascii_lowercase().as_str(),
...
#[cfg(feature = "metal")]
pub fn depthwise_conv1d_bias_swish_enabled_for_device(device: &candle_core::Device) -> bool {
    device.is_metal()
        && std::env::var("IZWI_PARAKEET_METAL_CONV1D_BN_SWISH")
            .ok()
            .map(|value| {
                matches!(
                    value.trim().to_ascii_lowercase().as_str(),
...
pub fn try_depthwise_conv2d(
    ...
    if !depthwise_conv2d_enabled_for_device(input.device()) || !weight.device().is_metal() {
        return None;
    }
...
#[cfg(feature = "metal")]
pub fn try_depthwise_conv1d_bias_swish(
    input: &Tensor,
    ...
    if input.dtype() != DType::F32
        || folded_weight.dtype() != DType::F32
        || folded_bias.dtype() != DType::F32
    {
        return None;
    }
    if !depthwise_conv1d_bias_swish_enabled_for_device(input.device())
        || !folded_weight.device().is_metal()
        || !folded_bias.device().is_metal()
    {
        return None;
    }
    let (batch, channels, input_width) = input.dims3().ok()?;
    let (out_channels, in_per_group, kernel_width) = folded_weight.dims3().ok()?;
    let bias_channels = folded_bias.dims1().ok()?;
    if batch == 0
        || channels == 0
        || input_width == 0
        || out_channels != channels
    ...
    let folded_weight = if folded_weight.is_contiguous() {
        folded_weight.clone()
    } else {
        folded_weight.contiguous().ok()?
    };
    let folded_bias = if folded_bias.is_contiguous() {
        folded_bias.clone()
    } else {
        folded_bias.contiguous().ok()?
    };
...
pub fn depthwise_conv1d_enabled_for_device(_device: &candle_core::Device) -> bool {
    false
}

#[cfg(not(feature = "metal"))]
pub fn depthwise_conv2d_enabled_for_device(_device: &candle_core::Device) -> bool {
    false
}

#[cfg(not(feature = "metal"))]
pub fn depthwise_conv1d_bias_swish_enabled_for_device(_device: &candle_core::Device) -> bool {
    false
}
...
pub fn try_depthwise_conv1d_bias_swish(
    _input: &candle_core::Tensor,
    _folded_weight: &candle_core::Tensor,
    _folded_bias: &candle_core::Tensor,
    _padding: usize,
    _stride: usize,
    _dilation: usize,
) -> Option<candle_core::Tensor> {
...
mod tests {
    ...
    #[test]
    fn depthwise_conv2d_falls_back_on_cpu() {
        let device = Device::Cpu;
        ...
        assert!(try_depthwise_conv2d(&input, &weight, 0, 1, 1).is_none());
    }

    #[test]
    fn depthwise_conv1d_bias_swish_falls_back_on_cpu() {
        let device = Device::Cpu;
        let input = Tensor::from_vec(vec![1.0f32, 2.0, 3.0, 4.0], (1, 1, 4), &device).unwrap();
        let weight = Tensor::from_vec(vec![0.25f32, 0.5, 0.25], (1, 1, 3), &device).unwrap();
        let bias = Tensor::from_vec(vec![0.125f32], 1, &device).unwrap();

        assert!(try_depthwise_conv1d_bias_swish(&input, &weight, &bias, 1, 1, 1).is_none());
    }
    ...
    fn depthwise_conv2d_metal_matches_candle_reference() -> candle_core::Result<()> {
        ...
        let reference = input.conv2d(&weight, 1, 2, 1, 3)?;
        let actual = try_depthwise_conv2d(&input, &weight, 1, 2, 1)
            .expect("metal depthwise conv2d should run");
    ...
    fn depthwise_conv1d_bias_swish_metal_matches_reference() -> candle_core::Result<()> {
        ...
        let weight_values: Vec<f32> = (0..10).map(|idx| (idx as f32 - 3.0) * 0.05).collect();
        let bias_values: Vec<f32> = vec![-0.25, 0.125];
        let input = Tensor::from_vec(input_values, (2, 2, 10), &device)?;
        let weight = Tensor::from_vec(weight_values, (2, 1, 5), &device)?;
        let bias = Tensor::from_vec(bias_values, 2, &device)?;

        let reference = input
            .conv1d(&weight, 2, 1, 1, 2)?
            .broadcast_add(&bias.reshape((1, 2, 1))?)?;
        let reference = reference.broadcast_mul(&candle_nn::ops::sigmoid(&reference)?)?;
        let actual = try_depthwise_conv1d_bias_swish(&input, &weight, &bias, 2, 1, 1)
            .expect("metal fused depthwise conv1d should run");
