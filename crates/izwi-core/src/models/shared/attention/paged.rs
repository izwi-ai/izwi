...
#[cfg(test)]
pub use legacy::{append_to_pages, materialize_pages, paged_decode_attention, KvPage};
...
mod legacy {
    ...
    fn dequantize_tensor_int8(values: &Tensor, scale: f32, target_dtype: DType) -> Result<Tensor> {
        ...
        let scale_t =
            Tensor::from_vec(vec![scale], (1,), values.device())?.to_dtype(target_dtype)?;
    ...
    fn q4_0_quantization_allowed_on_device(
        is_cuda: bool,
    ...
    fn paged_decode_attention_with_group_size(
        ...
        let running_out = running_out.ok_or_else(|| {
            Error::InferenceError(
                "Paged decode attention produced no valid page output".to_string(),
            )
        })?;
    ...
    mod tests {
        ...
        fn test_paged_decode_matches_dense_single_token() {
            ...
            let paged =
                paged_decode_attention(&q, &k_pages, &v_pages, num_heads, num_heads, head_dim)
                    .unwrap();
