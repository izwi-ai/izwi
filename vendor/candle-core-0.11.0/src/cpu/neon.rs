...
mod fp16 {
    ...
    //
    // Izwi patch: Candle 0.11.0's optimized Apple Silicon fp16 path uses the
    // unstable stdarch float16 vector type `float16x8_t` on stable Rust. Use
    // this existing widening fallback on macOS ARM while keeping native fp16
    // instructions available to the rest of the workspace.
    #[cfg(any(not(target_feature = "fp16"), target_os = "macos"))]
    mod inner {
        use super::*;
    ...
    #[cfg(all(target_feature = "fp16", not(target_os = "macos")))]
    mod inner {
        use super::*;
