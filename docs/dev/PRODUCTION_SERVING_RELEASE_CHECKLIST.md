...
| PSR-005 | High | Durable idempotency | The store has bounded tenant-scoped reservation/commit/replay primitives and atomically accepts a text-only TTS job graph with its optional committed result; worker attempt IDs and job attempt tokens fence execution writers | Do not promise public durable replay until route integration and crash/restart behavior pass without broadening the supported request shape |
