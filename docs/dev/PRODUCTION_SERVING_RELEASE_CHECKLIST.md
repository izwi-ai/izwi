...
| PSR-005 | High | Durable idempotency | Non-streaming text-only TTS now has bounded tenant-scoped public replay and atomic graph acceptance; keyed streaming/reference/saved-voice shapes reject before side effects | Do not broaden the replay promise until each additional request shape has crash/restart, input-publication, and cleanup evidence |
| PSR-006 | Medium | Durable cancellation UI/projections | Store retains requested/stopping ownership, fences results until exact teardown, and transactionally reconciles terminal route projections | Network-provider route-key serialization remains required before claiming concurrent multi-gateway projection correctness |
| PSR-007 | Medium | Tenant concurrency | Gateway has bounded fail-fast per-tenant/total accepted-work ownership and retains it across timeout/disconnect until exact terminal or teardown proof; worker admission remains authoritative | Process loss discards this authority; do not claim strict multi-gateway or crash-preserved tenant concurrency until Phase 8 ownership passes restart/partition tests |
| PSR-008 | Medium | Observability | Gateway and worker expose authenticated fixed-cardinality bounded metrics; the supervisor emits a bounded redacted Unix-signal snapshot; structured request logs and local runtime metrics remain available | Validate alert thresholds, scrape integration, and hardware utilization before unattended production operation |
| PSR-009 | Medium | Artifact lifecycle | Opaque tenant-scoped storage validates size/type/digest; creation durably retains a bounded exact provider-request envelope before I/O; deletion atomically tombstones and records a bounded durable retry intent; local maintenance processes both | Migrate media/speech provider writes, add retention and exact-teardown losing-attempt cleanup, and validate remote artifact transport before advertising artifact-dependent remote routes |
