...
| PSR-009 | Medium | Artifact lifecycle | Opaque tenant-scoped storage validates size/type/digest; deletion atomically tombstones and records a bounded durable retry intent processed by local maintenance | Add provider-write reservations, retention/orphan and exact-teardown losing-attempt cleanup, route migration, and remote artifact transport before advertising artifact-dependent remote routes |
