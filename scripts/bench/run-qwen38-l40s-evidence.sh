...
output_dir="${repo_root}/target/qwen38-l40s-evidence"
...
usage() {
    cat <<'EOF'
Usage: scripts/bench/run-qwen38-l40s-evidence.sh [options]
...
requires an observed NVIDIA L40S, an exact-SHA CUDA server, and measured TTFT
and completion throughput for every imported workload case.
EOF
}
...
if ! jq -e '
    .schema == "izwi.qwen38-l40s-workload.v1" and
...
' "${workload}" >/dev/null; then
    echo "Invalid or incomplete Qwen3.8 L40S workload: ${workload}" >&2
    exit 2
fi
...
write_certificate() {
    ...
    jq -n --arg status "${status}" --arg reason "${reason}" \
        ...
        '{schema:"izwi.qwen38-l40s-evidence.v1",status:$status,reason:$reason,
          run:{git_sha:$git_sha,workload:$workload,workload_sha256:$workload_sha256,
               started_at:$started_at,ended_at:$ended_at},device:null,measurements:null,
...
if [[ "${dry_run}" -eq 1 ]]; then
    ...
    echo "Qwen3.8 L40S evidence dry run: ${certificate_path}"
    exit 0
fi
if ! command -v "${nvidia_smi}" >/dev/null 2>&1 || ! "${nvidia_smi}" -L >/dev/null 2>&1; then
    ...
    echo "Qwen3.8 L40S evidence requires a device visible to nvidia-smi" >&2
    exit 1
fi

device_names=$("${nvidia_smi}" --query-gpu=name --format=csv,noheader 2>/dev/null || true)
if ! grep -Eiq '(^|[[:space:]])L40S([[:space:]]|$)' <<<"${device_names}"; then
    write_certificate failed nvidia_l40s_not_observed
    echo "Qwen3.8 L40S evidence requires an observed NVIDIA L40S" >&2
    exit 1
fi
...
if ! jq -e --arg model "${model}" --slurpfile plan "${workload}" \
    --slurpfile cuda_certificate "${cuda_output}/certificate.json" '
    .schema_version == 1 and
    ($cuda_certificate[0].status == "passed") and
    ($cuda_certificate[0].device.name | test("(^|[[:space:]])L40S([[:space:]]|$)"; "i")) and
...
jq -n --arg git_sha "${git_sha}" --arg workload "${workload}" \
    ...
    '{schema:"izwi.qwen38-l40s-evidence.v1",status:"passed",reason:"measured_qwen38_l40s_evidence_passed",
      run:{git_sha:$git_sha,workload:$workload,workload_sha256:$workload_sha256,
           checkpoint_revision:$plan[0].checkpoint.revision,started_at:$started_at,ended_at:$ended_at},
      device:$cuda_certificate[0].device,
...
echo "Qwen3.8 L40S evidence passed: ${certificate_path}"
