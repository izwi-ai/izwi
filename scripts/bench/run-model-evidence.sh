...
output_dir=""
izwi_bin="${IZWI_MODEL_EVIDENCE_IZWI:-${IZWI_CUDA_EVIDENCE_IZWI:-${repo_root}/target/release/izwi}}"
...
usage() {
    cat <<'EOF'
Usage: scripts/bench/run-model-evidence.sh --backend cpu|metal|cuda --manifest PATH [options]

Options:
  --server URL          Izwi server URL (default: http://127.0.0.1:8080)
  --backend BACKEND     Required runtime backend: cpu, metal, or cuda
  --output DIR          Evidence bundle directory
  --izwi-bin PATH       Izwi CLI binary
  --allow-remote        Permit a non-loopback server URL
  --allow-unsupported   Unavailable requested hardware emits unsupported instead of failing
  --require-optimized-kernel-evidence
                        Require every case to observe a fused/paged/RoPE backend path
...
Real certification is fail-closed by default. It requires exact requested and
selected backend health, strict benchmark quality, and per-case device telemetry.
EOF
}
...
if [[ -z "${output_dir}" ]]; then
    output_dir="${repo_root}/target/${backend}-model-evidence"
fi
...
if [[ "${allow_remote}" -ne 1 ]]; then
    case "${server}" in
        ...
        *) fail "Model evidence requires a loopback server unless --allow-remote is set" ;;
    esac
fi

if [[ "${backend}" == cuda ]] &&
   { ! command -v "${nvidia_smi}" >/dev/null 2>&1 || ! "${nvidia_smi}" -L >/dev/null 2>&1; }; then
...
if [[ "${worktree_clean}" != true ]]; then
    fail "Model certification requires a clean exact-SHA worktree"
fi
...
if ! jq -e --arg backend "${backend}" '
    ...
    .runtime.requested_backend == $backend and
    .runtime.requested_backend_available == true and
    .runtime.selected_backend == $backend and
    .runtime.compiled_backends[$backend] == true and
    (if $backend == "cuda" then
       .runtime.cuda_runtime.driver_available == true and
       .runtime.cuda_runtime.device_usable == true
     else true end)
' "${health_path}" >/dev/null; then
    fail "Izwi health does not prove the requested selected ${backend} runtime"
fi
...
' "${report_path}" >/dev/null; then
    fail "${backend} benchmark report failed case, sample, quality, runtime, batching, prefill, or optimized-kernel validation"
fi
if ! jq -e --arg git_sha "${git_sha}" '.schema_version == 1 and .git_sha == $git_sha' \
    "${metadata_path}" >/dev/null; then
    fail "${backend} benchmark metadata is not bound to the checked-out git SHA"
fi
if ! jq -e --arg backend "${backend}" '
    [.after.metrics | .. | objects |
        select(.backend_kind? == $backend and .actual_device_kind? == $backend)]
    | length > 0
' "${observability_path}" >/dev/null; then
    fail "Benchmark telemetry does not contain an observed ${backend} model"
fi
...
jq -n \
    ...
    '{
        schema: "izwi.model-evidence.v2",
        status: "passed",
        reason: "strict_model_evidence_passed",
        backend: $backend,
        run: {
            git_sha: $git_sha,
            manifest: $manifest,
        ...
        device: {
            ...
            backend_compiled: $health[0].runtime.compiled_backends[$backend],
            driver_available: (if $backend == "cuda" then $health[0].runtime.cuda_runtime.driver_available else true end),
            device_usable: (if $backend == "cuda" then $health[0].runtime.cuda_runtime.device_usable else true end),
...
echo "${backend} model evidence passed: ${certificate_path}"
